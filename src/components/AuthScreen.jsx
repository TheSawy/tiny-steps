...
// Simple obfuscation — prevents plain text in localStorage, not crypto
const obfuscate = (str) => btoa(unescape(encodeURIComponent(str)));
const deobfuscate = (str) => { try { return decodeURIComponent(escape(atob(str))); } catch { return null; } };

const CREDS_KEY = "ts_creds";
export const saveCredentials = (email, password) => {
  localStorage.setItem(CREDS_KEY, JSON.stringify({ e: obfuscate(email), p: obfuscate(password) }));
};
export const loadCredentials = () => {
  try {
    ...
    return { email: deobfuscate(e), password: deobfuscate(p) };
  } catch { return null; }
};
export const clearCredentials = () => localStorage.removeItem(CREDS_KEY);
